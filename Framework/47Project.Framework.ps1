...
function Show-47Gui {
  ...
  $
    # Update notifier (offline)
    $root.Children.Add((New-47Card 'Local Updates' 'Detects pack zips dropped into pack_updates/ (offline).' {
      $sp = New-Object System.Windows.Controls.StackPanel
      $updates = @(Get-47LocalUpdates)
      if ($updates.Count -eq 0) {
        $t = New-Object System.Windows.Controls.TextBlock
        $t.Text = 'No local updates detected.'
        $t.Foreground = $muted
        $sp.Children.Add($t) | Out-Null
      } else {
        $u = $updates[0]
        $t = New-Object System.Windows.Controls.TextBlock
        $t.Text = ('Found: ' + $u.Name + '  (' + $u.LastWriteTime.ToString('s') + ')')
        $t.Foreground = $fg
        $t.TextWrapping = 'Wrap'
        $sp.Children.Add($t) | Out-Null
        ...
        $btns = New-Object System.Windows.Controls.WrapPanel
        $btns.Margin = '0,10,0,0'
        $btns.Children.Add((New-47Button 'Open Updates Folder' {
          try { Start-Process (Join-Path (Get-47ProjectRoot) 'pack_updates') | Out-Null } catch { }
        })) | Out-Null
        $btns.Children.Add((New-47Button 'Open Pack Manager' { $nav.SelectedItem = 'Pack Manager' })) | Out-Null
        $sp.Children.Add($btns) | Out-Null
      }
      return $sp
    })) | Out-Null
...
$pages['Plans'] = {
  ...
  $pages['Module Wizard'] = {
    ...
    $root.Children.Add((New-47Card 'Module Generator' 'Create a new module skeleton under modules/<id> with module.json and stubs.' {
      ...
      $btns.Children.Add((New-47Button 'Generate Module' {
        GuiRun 'Generate module' {
          ...
          $meta = [pscustomobject]@{
            ...
            minPowerShellVersion = $minps
            entryScript = 'Invoke.ps1'
          }
          ($meta | ConvertTo-Json -Depth 6) | Set-Content -LiteralPath (Join-Path $dir 'module.json') -Encoding utf8

          $stub = @()
          $stub += '<#'
          $stub += '.SYNOPSIS'
          $stub += ('  ' + $name + ' module entry.')
          $stub += '.DESCRIPTION'
          $stub += '  Generated by Module Wizard.'
          $stub += '#>'
          $stub += ''
          $stub += 'param()'
          $stub += ''
          $stub += 'Write-Host "Module running: ' + $id + '"'
      ...
      $btns.Children.Add((New-47Button 'Open Modules Folder' {
        try { Start-Process (Join-Path (Get-47ProjectRoot) 'modules') | Out-Null } catch { }
      })) | Out-Null
...
foreach ($k in @('Home','Status','Plans','Modules','Settings','Trust','Bundles','Pack Manager','Verify','Config','Snapshots','Support','Doctor','Apps','Tasks','About','Module Wizard')) { [void]$nav.Items.Add($k) }
  $nav.SelectedIndex = 0
